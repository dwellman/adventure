...
public class AsciiRenderer implements Renderer {
    ...
    public String render(Table table) {
        ...
        columnWidths = applyMaxWidth(table, style, rowNumberWidth, columnWidths);
        ...
        if (style.showTitle() && table.title() != null && !table.title().isBlank()) {
            builder.append(fullWidthLine(
                    border.topLeft, border.horizontal, border.topRight, rowNumberWidth, padding, columnWidths
            )).append("\n");
        } else {
            builder.append(horizontalLine(
                    border.topLeft, border.headerSeparator, border.topRight, border.horizontal,
                    rowNumberWidth, padding, columnWidths
            )).append("\n");
        }
        ...
        if (style.showTitle() && table.title() != null && !table.title().isBlank()) {
            ...
            builder.append(horizontalLine(
                    border.leftSeparator, border.headerSeparator, border.rightSeparator, border.horizontal,
                    rowNumberWidth, padding, columnWidths)
            ).append("\n");
        }
        ...
        if (style.showHeader()) {
            ...
            builder.append(horizontalLine(
                    border.leftSeparator, border.intersection, border.rightSeparator, border.horizontal,
                    rowNumberWidth, padding, columnWidths
            )).append("\n");
        }
        ...
        for (int rowIndex = 0; rowIndex < table.rows().size(); rowIndex++) {
            ...
            if (rowIndex < table.rows().size() - 1 && style.showRowSeparators()) {
                builder.append(horizontalLine(
                        border.leftSeparator, border.intersection, border.rightSeparator,
                        border.horizontal, rowNumberWidth, padding, columnWidths
                ));
                builder.append("\n");
            }
        }
        ...
        builder.append(horizontalLine(
                border.bottomLeft, border.bottomCenter, border.bottomRight, border.horizontal, rowNumberWidth,
                padding, columnWidths
        ));
    ...
    private int[] computeColumnWidths(Table table, RenderStyle style) {
        ...
        for (int i = 0; i < table.columns().size(); i++) {
            ...
            String formattedTitle = InlineMarkdown.format(column.title());
            int width = Math.max(TextUtils.visibleLength(formattedTitle), column.minWidth());
            boolean wrapColumn = style.wrapCells() && column.wrap();
            for (Row row : table.rows()) {
                Cell cell = row.cells().get(i);
                String formatted = InlineMarkdown.format(
                        cell.render(table.formatterRegistry(), column.formatter()),
                        MarkdownStyleMap.tableCellStyle()
                );
                List<String> lines;
                if (wrapColumn) {
                ...
                for (String line : lines) {
                    int lineLength = TextUtils.visibleLength(line);
                    width = Math.max(width, Math.min(column.maxWidth(), lineLength));
                }
            }
    ...
    private String horizontalLine(
            String left, String intersect, String right, String lineChar, int rowNumberWidth,
            int padding, int[] columnWidths
    ) {
        StringBuilder builder = new StringBuilder();
        builder.append(left);
    ...
    private String titleLine(
            String title, RenderStyle style, int[] columnWidths, int rowNumberWidth
    ) {
        BorderCharacters border = style.border();
        int separators = Math.max(0, columnWidths.length - 1);
        int totalWidth = IntStream.of(
                columnWidths).sum() + (style.cellPadding() * 2 * columnWidths.length
        ) + separators;
        if (rowNumberWidth > 0) {
            totalWidth += rowNumberWidth + style.cellPadding() * 2 + 1; // include separator after row numbers
        }
        String formattedTitle = InlineMarkdown.format(title);
        String paddedTitle = TextUtils.pad(TextUtils.truncate(formattedTitle, totalWidth), totalWidth, Alignment.CENTER);
    ...
    private String headerLine(Table table, int padding, int rowNumberWidth, int[] columnWidths) {
        ...
        for (int i = 0; i < table.columns().size(); i++) {
            Column column = table.columns().get(i);
            builder.append(" ".repeat(padding));
            String title = InlineMarkdown.format(column.title());
            String clipped = TextUtils.truncate(title, columnWidths[i]);
            builder.append(TextUtils.pad(clipped, columnWidths[i], column.alignment()));
            builder.append(" ".repeat(padding));
            builder.append(border.vertical);
        }
    ...
    private int[] applyMaxWidth(Table table, RenderStyle style, int rowNumberWidth, int[] columnWidths) {
        int maxWidth = style.maxWidth();
        if (maxWidth <= 0 || columnWidths.length == 0) {
            return columnWidths;
        }
        int totalWidth = tableWidth(columnWidths, style, rowNumberWidth);
        if (totalWidth <= maxWidth) {
            return columnWidths;
        }
        int[] adjusted = columnWidths.clone();
        int[] minWidths = columnMinWidths(table);
        int overflow = totalWidth - maxWidth;
        while (overflow > 0) {
    ...
    private int[] columnMinWidths(Table table) {
        int[] minWidths = new int[table.columns().size()];
        for (int i = 0; i < table.columns().size(); i++) {
            minWidths[i] = Math.max(1, table.columns().get(i).minWidth());
        }
        return minWidths;
    }

    private int tableWidth(int[] columnWidths, RenderStyle style, int rowNumberWidth) {
        int separators = Math.max(0, columnWidths.length - 1);
        int total = IntStream.of(columnWidths).sum() + (style.cellPadding() * 2 * columnWidths.length) + separators;
        if (rowNumberWidth > 0) {
            total += rowNumberWidth + style.cellPadding() * 2 + 1;
        }
        return total + 2;
    }

    private String dataRow(
            int rowIndex, Row row, Table table, int padding, int rowNumberWidth, int[] columnWidths
    ) {
        BorderCharacters border = table.style().border();
        RenderStyle style = table.style();
    ...
    private String fullWidthLine(
            String left, String lineChar, String right, int rowNumberWidth, int padding, int[] columnWidths
    ) {
        StringBuilder builder = new StringBuilder();
        builder.append(left);
        int separators = Math.max(0, columnWidths.length - 1);
