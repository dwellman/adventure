...
public final class NarratorPromptBuilder {
    ...
    // Pattern: Grounding
    // - Uses only the engine output and snapshot fields to anchor narration in current game state.
    public static String buildEngine(String playerUtterance,
        ...
        return template.formatted(
                ...
                sceneDetailLevel(rawEngineOutput, sceneSnapshot),
                nullToEmpty(backstory),
                nullToEmpty(rawEngineOutput),
                nullToEmpty(sceneSnapshot),
                ""
        );
    }

    // Pattern: Grounding
    // - Builds narration from the last snapshot + color event without relying on hidden state.
    public static String buildSnapshot(String playerUtterance,
        ...
        return template.formatted(
                nullToEmpty(playerUtterance),
                nullToEmpty(canonicalCommand),
                sceneDetailLevel("", sceneSnapshot),
                nullToEmpty(backstory),
                "",
                nullToEmpty(sceneSnapshot),
    ...
    private static String loadTemplate() {
        String template = PromptTemplates.load("agents/narrator.md");
        return template == null ? defaultUnifiedTemplate() : template;
    }

    private static String defaultUnifiedTemplate() {
        return """
