...
class IslandAdventureYamlTest {

    @TempDir
    Path tempDir;

    @Test
    void writesIslandAdventureYamlDeterministically() throws Exception {
        Path canonical = Path.of("src/main/resources/cookbook/gardened-island.yaml");
        assertThat(canonical).exists();
        GameSave canonicalSave = GameSaveYamlLoader.load(canonical);

        Path file = tempDir.resolve("island-adventure.yaml");
    ...
    void loadsCanonicalYamlAndMatchesRecipes() throws Exception {
        ...
        Path file = tempDir.resolve("island-roundtrip.yaml");
