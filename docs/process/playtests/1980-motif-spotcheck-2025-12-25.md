# 1980 Motif Verb Spot Check - 2025-12-25

Scope: quick 1980-mode run using motif verbs to confirm alias parsing and basic movement.
Note: transcript output normalized to ASCII for plain-text logging (smart quotes/bullets + box drawing converted).

## Island Adventure

Commands: `1`, `look`, `scavenge`, `read rags`, `paddle east`, `look`, `row west`, `quit`
...
_ 
      Wreck Beach The golden sands of Wreck Beach stretch invitingly, dotted
  with remnants of past adventures and whispers of the ocean's secrets.
  Items:
...
_ 
  Rags: Salt-crusted, torn cloth strips -- perfect for wrapping a torch head
  and soaking fuel.

_ 
  Tidepool Rocks
  The Tidepool Rocks stretch out like ancient sentinels, their weathered
  surfaces glistening in the sun, inviting explorers to uncover the secrets
  hidden within the shimmering pools below.
...
Commands: `2`, `look`, `examine case board`, `probe`, `west`, `look`, `east`, `quit`
...
Commands: `3`, `look`, `scout`, `ride east`, `look`, `trot west`, `quit`
...
_ 
      Train Platform Steam, coal smoke, and a long string of cars idling for
  the last run out of Dry Gulch. Items:

  - Station Key
      Exits: EAST, NORTH (A padlocked freight door with a brass tag), SOUTH,
  WEST
...
_ 
      Signal Shack A lean shed with a signal lever and a view of the canyon
  line. Fixtures:
...
Commands: `4`, `look`, `surveil`, `sneak east`, `scan`, `tail west`, `quit`
