...
- Tests: `mvn -q test`
- Scope: Repo-wide deep clean (housekeeping docs/logs, README/docs updates, stale TODO cleanup, reserved scanner helper noted).
- Tests: `mvn -q test`
- Scope: Docs cleanup (moved zone-input sample into src/test/resources/zone-demo, moved Island backstory into `src/main/resources/storybook/island` with narrative-only placeholders, added Key Expression syntax doc under docs/design, updated references).
- Tests: Not run (docs/storybook refactor).
- Scope: Docs reorg (moved design docs into docs/design, moved walkthrough into docs/guides, kept docs root to README/journal), and converted prompt templates from .txt to .md with updated gardener expander path.
- Tests: Not run (docs/prompt refactor).
- Scope: Docs reorg (renamed docs/architecture to docs/design, moved index content to docs/index.md, simplified docs/readme.md).
...
- Scope: Logged full 1980 playthroughs for Island, Mansion, and Western adventures in `docs/scoreboard.md` with runtime and blocking issues.
...
- Scope: Logged a full 1980 Spy Adventure playthrough in `docs/scoreboard.md` with runtime and blocking issues.
...
- Tests: `printf '1\nlook\nscavenge\nread rags\npaddle east\nlook\nrow west\nquit\n' | ./adventure --mode=1980 --quiet`; `printf '2\nlook\nexamine case board\nprobe\nwest\nlook\neast\nquit\n' | ./adventure --mode=1980 --quiet`; `printf '3\nlook\nscout\nride east\nlook\ntrot west\nquit\n' | ./adventure --mode=1980 --quiet`; `printf '4\nlook\nsurveil\nsneak east\nscan\ntail west\nquit\n' | ./adventure --mode=1980 --quiet`
- Scope: Captured ASCII-normalized 1980 motif-verb spot check transcripts for all adventures under `docs/playtests/1980-motif-spotcheck-2025-12-25.md`.
- Tests: `./adventure --mode=1980 --quiet` runs captured in `docs/playtests/1980-motif-spotcheck-2025-12-25.md`
...
- Tests: `./adventure --mode=1980 --quiet < /tmp/island-1980-win-input.txt > docs/playtests/2025-12-26-island-1980-walkthrough.md`; `./adventure --mode=1980 --quiet < /tmp/mansion-1980-win-input.txt > docs/playtests/2025-12-26-mansion-1980-walkthrough.md`; `./adventure --mode=1980 --quiet < /tmp/western-1980-win-input.txt > docs/playtests/2025-12-26-western-1980-walkthrough.md`
...
- Tests: `./adventure --mode=1980 --quiet < /tmp/spy-1980-win-input.txt > docs/playtests/2025-12-26-spy-1980-walkthrough.md`

## 2025-12-26 — Docs/runbook reorg
- Scope: Moved root-level docs into `docs/`, added a logs hub, and consolidated AI/CLI runbooks under `docs/runbooks/`.
- Tests: none
